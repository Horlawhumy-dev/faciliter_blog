...
def read_posts(skip: int = 0, limit: int = 10, search: Optional[str] = None, author_id: int = 1, db: Session = Depends(get_db)):
    posts = crud.post.get_posts(db, skip=skip, limit=limit, search=search, author_id=author_id)
    return posts

@router.get("/{post_id}", response_model=Post)
def read_post(post_id: int, author_id:int, db: Session = Depends(get_db)):
...
def update_post(post_id: int, author_id:int, post: PostCreate, db: Session = Depends(get_db)):
